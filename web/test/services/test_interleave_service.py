...
def test_interleave_rankings(sessions):
    ...
    # The interleaved dict is added to the Result object which is a JSON type. Therefore the keys are converted from int to str. Here we revert this conversion.
...
def test_interleave_rankings_more_results_than_expected(sessions):
    result = create_results(sessions)
    ...
    results_exp.items = {
        ...
        2: {"docid": "doc12", "type": "EXP"},
        3: {"docid": "doc13", "type": "EXP"},
        4: {"docid": "doc14", "type": "EXP"},
        5: {"docid": "doc15", "type": "EXP"},
    }

    interleaved_results = interleave_rankings(
        results_exp, results_base, "ranking", rpp=2
    )

    assert len(interleaved_results.items) == 2
...
class TestInterleaving:
    ...
    def test_team_draft_distribution(self):
        ...
        assert sum(pattern_counts.values()) == iterations
