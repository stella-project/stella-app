...
from app.services.interleave_service import interleave_rankings,interleave_recommendations
...
def build_response(
    ...
    id_map = build_id_map(
        current_app.config["RECOMMENDATION_BASELINE_CONTAINER"], recommendation_base, result_base
    )
...
def make_recommendation(container_name, item_id, rpp, page, session_id):
    ...
    if current_app.config["INTERLEAVE"]:
        ...
        recommendation_base, result_base = query_system(
            current_app.config["RECOMMENDATION_BASELINE_CONTAINER"],
            item_id,
            rpp,
            page,
            session_id,
        ...
        interleaved_recommendation = interleave_recommendations(recommendation, recommendation_base)
